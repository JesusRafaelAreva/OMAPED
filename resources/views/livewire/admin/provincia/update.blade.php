<div class="card">
    <div class="card-header p-0">
        <h3 class="card-title">{{ __('UpdateTitle', ['name' => __('Provincia') ]) }}</h3>
        <div class="px-2 mt-4">
    ...
    <form class="form-horizontal" wire:submit.prevent="update" enctype="multipart/form-data">
        ...
        <div class="card-body">
            ...
            <!-- IdDepa Input -->
            <div class='form-group'>
                <label for='input-idDepa' class='col-sm-2 control-label '> {{ __('IdDepa') }}</label>
                <select id='input-idDepa' wire:model.lazy='idDepa' class="form-control  @error('idDepa') is-invalid @enderror">
                    @foreach(getCrudConfig('Provincia')->inputs()['idDepa']['select'] as $key => $value)
